...
module riscv_cache_soc
     #(parameter BOOT_VECTOR            = 32'h0000_0000
      ...
      ,parameter ADDR_I2C_A = 32'h9005_0000
      ...
      ,parameter SIZE_I2C_A = 32'h0000_1000
      ,parameter NUM_IRQ    = 5// timer(0), uart(1), gpio(2), i2c(3)
      ,parameter IRQ_TIMER  = 0  // timer(0)
      ,parameter IRQ_UART   = 1  // uart(1)
      ,parameter IRQ_GPIO   = 2  // gpio(2)
      ,parameter IRQ_I2C    = 3  // i2c(3)
      ,parameter IRQ_I2C_A  = 4
...
(
     ...
     (* mark_debug="true" *) input  wire  uart_cts_n, // tie 0 if not used
     (* mark_debug="true" *) input  wire [7:0] gpio_in,  // GPIO input pins
     (*  mark_debug="true" *) output wire [7:0] gpio_out, // GPIO output pins
     inout  wire        sda,  // I2C data line (for u_i2c)
     (* mark_debug="true" *) output wire        scl,  // I2C clock line (for u_i2c)
     inout  wire        sda_a,  // I2C data line (for u_i2c_a - new I2C)
     inout  wire        scl_a,  // I2C clock line (for u_i2c_a - new I2C)
    ...
    `ifdef CON_FMC
     ,
`ifndef __ICARUS__
     (* X_INTERFACE_PARAMETER = "ASSOCIATED_BUSIF s_axi_confmc,ASSOCIATED_RESET axi_aresetn,RUSER_WIDTH 0,WUSER_WIDTH 0,ARUSER_WIDTH 0,AWUSER_WIDTH 0,PROTOCOL AXI4,HAS_BURST 1,HAS_CACHE 0,HAS_LOCK 0,HAS_PROT 0,HAS_QOS 0,HAS_REGION 0,HAS_wstrb 1,HAS_bresp 1,HAS_rresp 1" *)
`endif
...
`endif
    ...
    initial begin
        ...
        if ((ADDR_GPIO <ADDR_PERIPHERAL)||((ADDR_GPIO +SIZE_GPIO )>(ADDR_PERIPHERAL+SIZE_PERIPHERAL))) begin
            $display("%m ERROR GPIO address.");
        end
        if ((ADDR_I2C <ADDR_PERIPHERAL)||((ADDR_I2C +SIZE_I2C )>(ADDR_PERIPHERAL+SIZE_PERIPHERAL))) begin
            $display("%m ERROR I2C address.");
        end
        if ((ADDR_I2C_A <ADDR_PERIPHERAL)||((ADDR_I2C_A +SIZE_I2C_A )>(ADDR_PERIPHERAL+SIZE_PERIPHERAL))) begin
            $display("%m ERROR I2C_A address.");
        end
    ...
    wire  [7:0]  gpio_dir;  // GPIO direction (internal signal)
    //--------------------------------------------------------------------------
   ...
   (* mark_debug="true" *)wire                      axi_inst_rready , axi_data_rready ;
    ...
    u_riscv_cache_core (
        ...
        ,.m_axi_inst_awsize  ( axi_inst_awsize  )
        ,.m_axi_inst_awburst ( axi_inst_awburst )
        ,.m_axi_inst_awvalid ( axi_inst_awvalid )
  ...
  //assign axi_bus_wid[0]=(axi_bus_awvalid[0]) ? axi_bus_awid[0] : reg_wid0;
    //--------------------------------------------------------------------------
    wire [31:0] axi_lite_awaddr [0:6];
    wire        axi_lite_awvalid[0:6];
    wire        axi_lite_awready[0:6];
    wire [31:0] axi_lite_wdata  [0:6];
    wire        axi_lite_wvalid [0:6];
    wire        axi_lite_wready [0:6];
    wire [ 1:0] axi_lite_bresp  [0:6];
    wire        axi_lite_bvalid [0:6];
    wire        axi_lite_bready [0:6];
    wire [31:0] axi_lite_araddr [0:6];
    wire        axi_lite_arvalid[0:6];
    wire        axi_lite_arready[0:6];
    wire [31:0] axi_lite_rdata  [0:6];
    wire [ 1:0] axi_lite_rresp  [0:6];
    wire        axi_lite_rvalid [0:6];
    wire        axi_lite_rready [0:6];
    //--------------------------------------------------------------------------
  ...
  //assign axi_bus_wid[1]=(axi_bus_awvalid[1]) ? axi_bus_awid[1] : reg_wid1;
    ...
    u_axi_lite_bus (
        ...
        , .m5_axi_lite_awaddr  ( axi_lite_awaddr [6])
        , .m5_axi_lite_awvalid ( axi_lite_awvalid[6])
        , .m5_axi_lite_awready ( axi_lite_awready[6])
        , .m5_axi_lite_wdata   ( axi_lite_wdata  [6])
        , .m5_axi_lite_wvalid  ( axi_lite_wvalid [6])
        , .m5_axi_lite_wready  ( axi_lite_wready [6])
        , .m5_axi_lite_bresp   ( axi_lite_bresp  [6])
        , .m5_axi_lite_bvalid  ( axi_lite_bvalid [6])
        , .m5_axi_lite_bready  ( axi_lite_bready [6])
        , .m5_axi_lite_araddr  ( axi_lite_araddr [6])
        , .m5_axi_lite_arvalid ( axi_lite_arvalid[6])
        , .m5_axi_lite_arready ( axi_lite_arready[6])
        , .m5_axi_lite_rdata   ( axi_lite_rdata  [6])
        , .m5_axi_lite_rresp   ( axi_lite_rresp  [6])
        , .m5_axi_lite_rvalid  ( axi_lite_rvalid [6])
        , .m5_axi_lite_rready  ( axi_lite_rready [6])
    );
    //--------------------------------------------------------------------------
    ...
    // //--------------------------------------------------------------------------
    // //---- I2C Debug Signals (for ILA) - READonly
    // (* mark_debug="true" *) wire [31:0] i2c_axi_araddr;   // READ 주소
    // (* mark_debug="true" *) wire        i2c_axi_arvalid;  // READ 요청
    // (* mark_debug="true" *) wire        i2c_axi_arready;  // READ 준비
    // (* mark_debug="true" *) wire [31:0] i2c_axi_rdata;    // READ 데이터
    // (* mark_debug="true" *) wire [ 1:0] i2c_axi_rresp;     // READ 응답
    // (* mark_debug="true" *) wire        i2c_axi_rvalid;   // READ 완료
    // (* mark_debug="true" *) wire        i2c_axi_rready;   // CPU 준비
    
    // // WRITE 신호는 mark_debug 제거 (리소스 절약)
    // wire [31:0] i2c_axi_awaddr;
    // wire        i2c_axi_awvalid;
    // wire        i2c_axi_awready;
    ...
    // assign i2c_axi_awaddr  = axi_lite_awaddr [5];
    // assign i2c_axi_awvalid = axi_lite_awvalid[5];
    // assign i2c_axi_awready = axi_lite_awready[5];
    // assign i2c_axi_wdata   = axi_lite_wdata  [5];
    // assign i2c_axi_wvalid  = axi_lite_wvalid [5];
    // assign i2c_axi_wready  = axi_lite_wready [5];
    // assign i2c_axi_bresp   = axi_lite_bresp  [5];
    // assign i2c_axi_bvalid  = axi_lite_bvalid [5];
    // assign i2c_axi_bready  = axi_lite_bready [5];
    // assign i2c_axi_araddr  = axi_lite_araddr [5];
    // assign i2c_axi_arvalid = axi_lite_arvalid[5];
    // assign i2c_axi_arready = axi_lite_arready[5];
    // assign i2c_axi_rdata   = axi_lite_rdata  [5];
    // assign i2c_axi_rresp   = axi_lite_rresp  [5];
    // assign i2c_axi_rvalid  = axi_lite_rvalid [5];
    // assign i2c_axi_rready  = axi_lite_rready [5];
    
    // i2c_axi_lite #(.CLK_FREQ(ACLK_FREQ))
    // u_i2c (
    //       .aresetn       ( axi_aresetn        )
    //     , .aclk          ( axi_aclk           )
    ...
    // );
    //--------------------------------------------------------------------------
    // New I2C Master Module (ex_i2c_master_axi_lite)
    // Independent I2C bus signals (sda_a, scl_a)
    // Tri-state buffer for SCL_A using IOBUF primitive
    // Note: IOBUF T signal: 1=high-Z (input), 0=output
    //       But I2C uses: T=1 means high-Z, T=0 means drive
    wire i2c_a_scl_i, i2c_a_scl_o, i2c_a_scl_t;
    IOBUF i2c_scl_buf (
        .O(i2c_a_scl_i),      // Buffer output (input from pin)
        .IO(scl_a),           // Buffer inout port (connect directly to top-level port)
        .I(i2c_a_scl_o),      // Buffer input (output to pin)
        .T(i2c_a_scl_t)       // 3-state enable: 1=high-Z (input), 0=output
    );
    
    // Tri-state buffer for SDA_A using IOBUF primitive
    wire i2c_a_sda_i, i2c_a_sda_o, i2c_a_sda_t;
    IOBUF i2c_sda_buf (
        .O(i2c_a_sda_i),      // Buffer output (input from pin)
        .IO(sda_a),           // Buffer inout port (connect directly to top-level port)
        .I(i2c_a_sda_o),      // Buffer input (output to pin)
        .T(i2c_a_sda_t)       // 3-state enable: 1=high-Z (input), 0=output
    );
    
    i2c_master_axi_lite #(.P_CLK_FREQ(ACLK_FREQ)
                         ,.P_I2C_SPEED(400_000)
                         ,.P_RX_DEPTH(8)
                         ,.P_TX_DEPTH(8))
    u_i2c_a (
          .aresetn          ( axi_aresetn        )
        , .aclk             ( axi_aclk           )
        , .s_axi_lite_awaddr  ( axi_lite_awaddr [6])
        , .s_axi_lite_awvalid ( axi_lite_awvalid[6])
        , .s_axi_lite_awready ( axi_lite_awready[6])
        , .s_axi_lite_wdata   ( axi_lite_wdata  [6])
        , .s_axi_lite_wvalid  ( axi_lite_wvalid [6])
        , .s_axi_lite_wready  ( axi_lite_wready [6])
        , .s_axi_lite_bresp   ( axi_lite_bresp  [6])
        , .s_axi_lite_bvalid  ( axi_lite_bvalid [6])
        , .s_axi_lite_bready  ( axi_lite_bready [6])
        , .s_axi_lite_araddr  ( axi_lite_araddr [6])
        , .s_axi_lite_arvalid ( axi_lite_arvalid[6])
        , .s_axi_lite_arready ( axi_lite_arready[6])
        , .s_axi_lite_rdata   ( axi_lite_rdata  [6])
        , .s_axi_lite_rresp   ( axi_lite_rresp  [6])
        , .s_axi_lite_rvalid  ( axi_lite_rvalid [6])
        , .s_axi_lite_rready  ( axi_lite_rready [6])
        , .IRQ               ( irq[IRQ_I2C_A]    )
        , .SCL_I             ( i2c_a_scl_i       )
        , .SCL_O             ( i2c_a_scl_o       )
        , .SCL_T             ( i2c_a_scl_t       )
