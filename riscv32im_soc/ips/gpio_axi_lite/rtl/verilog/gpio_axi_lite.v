...
module gpio_axi_lite
     #(parameter P_WIDTH=8) // bit-width for gpio
(
...
(
   ...
   , output wire               interrupt
    ...
    if (resetn==1'b0) begin
        ...
        if (ren==1'b1) begin
           case (raddr[7:0])
               CSRA_VERSION: rdata <= P_VERSION [7:0]; //버전 정보
               CSRA_NAME   : rdata <= P_NAME;    // 모듈 이름
               CSRA_DIR    : rdata <= csr_dir;   // 방향 제어
               CSRA_INPUT  : rdata <= gpio_in;   // 입력 상태
               CSRA_OUTPUT : rdata <= gpio_out;  // 출력 상태
               CSRA_MASK   : rdata <= csr_mask;  // 인터럽트 마스크
               CSRA_IRQ    : rdata <= csr_irq;   // 인터럽트 플래그
               CSRA_EDGE   : rdata <= csr_edge;  // 에지 센서 모드
               CSRA_POL    : rdata <= csr_pol;   // 인터럽트 극성
               default     : begin
                             rdata <= 'h0;
                             slverr_rd <= 1'b1;
                             end
    ...
    // CSR write
    always @ (posedge clk or negedge resetn) begin
      if (resetn==1'b0) begin // 리셋 활성화된 상황
          slverr_wr <= 1'b0;
          csr_dir   <= {P_WIDTH{1'b0}}; // all input
          gpio_out  <= {P_WIDTH{1'b0}}; // all output
          csr_mask  <= {P_WIDTH{1'b1}}; // all masked (i.e., no interrupt)
          csr_edge  <= {P_WIDTH{1'b0}}; // all level     #(parameter P_WIDTH=8) // bit-width for gpio
